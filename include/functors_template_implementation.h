#ifndef UNTITLED1_FUNCTORS_TEMPLATE_IMPLEMENTATION_H
#define UNTITLED1_FUNCTORS_TEMPLATE_IMPLEMENTATION_H

#include "functors.h"
#include "DSU.h"

template <bypass_type _type, typename T>
T D_or_B_FS<_type, T>::back(std::stack<T>& stack) {
    return stack.top();
}

template <bypass_type _type, typename T>
T D_or_B_FS<_type, T>::back(std::queue<T>& queue) {
    return queue.front();
}

template <bypass_type _type, typename T>
template <typename U>
std::vector<T> D_or_B_FS<_type, T>::get_one_component(U data_structure, const std::unordered_map<T, std::unordered_set<T>>& vertex_table,
...
template <bypass_type _type, typename T>
std::vector<std::vector<T>> D_or_B_FS<_type, T>::operator()(const std::unordered_map<T, std::unordered_set<T>>& vertex_table) {
...
template <typename T>
void Kosaraju<T>::dfs_for_ordering (const size_t& v, const std::unordered_map<T, std::unordered_set<T>>& vertex_table,
...
template <typename T>
void Kosaraju<T>::dfs_for_searching (const size_t& v, const std::unordered_map<T, std::unordered_set<T>>& vertex_table,
...
template <typename T>
std::vector<std::vector<T>> Kosaraju<T>::operator()(const std::unordered_map<T, std::unordered_set<T>>& vertex_table) {
...
template <typename T>
std::vector<std::vector<T>> DSU_for_components<T>::operator()(const std::unordered_map<T, std::unordered_set<T>>& vertex_table) {
    DSU<T> dsu;
...
#endif //UNTITLED1_FUNCTORS_TEMPLATE_IMPLEMENTATION_H
